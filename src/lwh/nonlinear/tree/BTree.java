...
public class BTree {
    ...
    //建立一颗二叉排序树
    public static BTree buildBST(int[] a) {
        ...
        for (int i = 0; i < a.length; i++) {
            insertNodeToBST(t, a[i]);
        }
        return t;
    }

    public static boolean insertNodeToBST(BTree t, int x) {
        Node node = new Node(x);
        if (t.root == null) {
            t.root = node;
            return true;
    ...
    //二叉排序树中查找
    //若存在返回的就是指向该值的p,否则返回该值的前驱节点
    public static Node findNode(BTree t, int x) {
    ...
    //二叉排序树中删除
    public static boolean deleteBSTNode(BTree t, int x) {
        Node node = t.root;
        Node parent = null;
        ...
        if (node.rchild == null) {
            ...
            //左右子树都不为空,可以将要删除节点按中序遍历的前驱或后继节点替代要删除节点的位置,此处取前驱节点
        } else {
            //取前驱结点,先转左,然后一直到最右
            Node replaceNode = node.lchild;
    ...
    //输入一个数组，重建二叉树，比如[2,3,1,4,7,5,6]，选最大的7作为根节点，7左边是左子树，右边是右子树，
    //再从左边[2,3,1,4]中选出最大的作为左子树的根节点，以此类推，输出一棵二叉树
    public static BTree buildTree(int[] a) {
        if (a == null || a.length == 0) {
            return null;
        }
        BTree tree = new BTree();
        tree.root = getBTree(a, 0, a.length - 1);
        return tree;
    }

    private static Node getBTree(int[] a, int left, int right) {
        int maxIndex = left;
        ...
        if (maxIndex > left) {
            root.lchild = getBTree(a, left, maxIndex - 1);
        } else {
            root.lchild = null;
        }

        if (maxIndex < right) {
            root.rchild = getBTree(a, maxIndex + 1, right);
        } else {
            root.rchild = null;
        }
